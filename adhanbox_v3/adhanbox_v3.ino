...
//Version: 3.0.5 (AdhanBox V3 / HW v3)
#include <Arduino.h>
...
void handleFirmwareVersion() {
  server.send(200, "application/json",
              "{\"version\":\"3.0.5\",\"hardware\":\"v3\",\"build\":\"" __DATE__ " " __TIME__ "\"}");
}
...
void handleDeviceInfo() {
  ...
  if (pairingWindow || hasValidToken) {
    snprintf(buf, sizeof(buf),
             "{\"version\":\"3.0.5\",\"hardware\":\"v3\",\"hostname\":\"%s\",\"device_id\":\"%s\",\"token\":\"%s\",\"ota_pass\":\"%s\"}",
             OTA_HOSTNAME, deviceIdHex().c_str(), _apiToken.c_str(), _otaPass.c_str());
  } else {
    snprintf(buf, sizeof(buf),
             "{\"version\":\"3.0.5\",\"hardware\":\"v3\",\"hostname\":\"%s\",\"device_id\":\"%s\",\"paired\":true}",
