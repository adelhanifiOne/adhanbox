// Données statiques du Coran : 114 sourates + récitateurs disponibles sur la SD.
// Les fichiers sont pré-chargés sur la carte : /quran/<slug>/NNN.mp3 (NNN = 001..114)

class Surah {
  final int number;
  final String name; // translittération FR
  final String arabic;
  const Surah(this.number, this.name, this.arabic);
}

class Reciter {
  final String slug; // dossier sur la SD : /quran/<slug>/
  final String name; // nom affiché
  const Reciter(this.slug, this.name);
}

// Récitateurs pré-chargés (doivent correspondre aux dossiers sur la SD)
const List<Reciter> kReciters = [
  Reciter('afs', 'Mishary Al-Afasy'),
  Reciter('ghamdi', 'Saad Al-Ghamdi'),
  Reciter('dosari', 'Yasser Al-Dosari'),
  Reciter('maher', 'Maher Al-Meaqli'),
];

// Chemin du fichier d'une sourate pour un récitateur donné
String surahPath(String reciterSlug, int surahNumber) =>
    '/quran/$reciterSlug/${surahNumber.toString().padLeft(3, '0')}.mp3';

const List<Surah> kSurahs = [
  Surah(1, 'Al-Fatiha', 'الفاتحة'),
  Surah(2, 'Al-Baqara', 'البقرة'),
  Surah(3, 'Al-Imran', 'آل عمران'),
  Surah(4, 'An-Nisa', 'النساء'),
  Surah(5, 'Al-Maida', 'المائدة'),
  Surah(6, 'Al-Anam', 'الأنعام'),
  Surah(7, 'Al-Araf', 'الأعراف'),
  Surah(8, 'Al-Anfal', 'الأنفال'),
  Surah(9, 'At-Tawba', 'التوبة'),
  Surah(10, 'Yunus', 'يونس'),
  Surah(11, 'Hud', 'هود'),
  Surah(12, 'Yusuf', 'يوسف'),
  Surah(13, 'Ar-Rad', 'الرعد'),
  ...
  Surah(60, 'Al-Mumtahana', 'الممتحنة'),
  Surah(61, 'As-Saff', 'الصف'),
  Surah(62, 'Al-Jumua', 'الجمعة'),
  ...
  Surah(69, 'Al-Haqqa', 'الحاقة'),
  Surah(70, 'Al-Maarij', 'المعارج'),
  Surah(71, 'Nuh', 'نوح'),
  ...
  Surah(82, 'Al-Infitar', 'الانفطار'),
  Surah(83, 'Al-Mutaffifin', 'المطففين'),
  Surah(84, 'Al-Inshiqaq', 'الانشقاق'),
